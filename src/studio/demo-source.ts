...
function demoPoses(time: number, cube: Vec3): { left: Pose; right: Pose } {
  ...
  const leftPresent: Pose = { wrist: [-0.16, 0, -0.55], curl: OPEN };
  const rightPresent: Pose = { wrist: [0.16, 0, -0.55], curl: POINT };
  ...
  if (u >= 9 && u < 11.5) {
    ...
    right = { wrist: [0.16, Math.sin(time * 2.2) * 0.015, -0.55], curl: lerpCurl(POINT, OPEN, k) };
  } else if (u >= 11.5) {
...
export function sampleDemo(timestamp: number, space: InteractionSpace = DEFAULT_SPACE, cube: Vec3 = [-0.35, 0.8, -0.95]): TrackingFrame {
