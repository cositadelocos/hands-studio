...
export class MediaPipeSource {
  ...
  async start(facing: FacingMode): Promise<void> {
    ...
    const stream = await navigator.mediaDevices.getUserMedia({
      audio: false,
      video: { facingMode: facing, width: { ideal: 192 }, height: { ideal: 144 }, frameRate: { ideal: 12, max: 15 } },
    });
    this.stream = stream;
  ...
  sample(now: number): TrackingFrame | null {
    ...
    this.detectGap = elapsed > 40 ? 240 : elapsed > 20 ? 150 : 100;
    this.revision += 1;
